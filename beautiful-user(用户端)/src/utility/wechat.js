...
export const weChatH5ShareFriend = async () => {
	return new Promise((resove, reject) => {
		const link = window.location.origin + '/user/';
		let Guid = store.state.userInfo?.userInfoGuid ? '?guid=' + store.state.userInfo?.userInfoGuid + '#/' : '';
		WX.ready(function () {
			WX.updateAppMessageShareData({
				title: '美优到家', // 分享标题
				desc: '美优到家是一家极速上门服务直约平台，提供专业，正规，安全，快捷的24小时上门服务。', // 分享描述
...
export const weChatH5ShareCircleOfFriends = async () => {
	return new Promise((resove, reject) => {
		const link = window.location.origin + '/user/';
		let Guid = store.state.userInfo?.userInfoGuid ? '?guid=' + store.state.userInfo?.userInfoGuid + '#/' : '';
		WX.ready(function () {
