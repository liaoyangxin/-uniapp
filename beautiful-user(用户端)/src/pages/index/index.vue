<template>
	<view class="content">
		<z-paging ref="paging" v-model="productList" @query="queryList" :auto="false">
			<view class="swiper">
				<u-swiper
					:list="bannerData"
					indicator
			...
			<view class="followStyle" v-if="followStatus == false">
				<u-icon name="bell" color="rgba(255, 255, 255, 1)" class="m-5"></u-icon>
				<text class="flex-no-horizontal" @click="followClick"
					>点击扫码关注美优到家公众号，以便给您及时送达消息！</text
				>
			</view>
			<view class="product">
		...
		<u-popup :show="showModel" @click="showModel = false" mode="center">
			<view class="model">
				...
				<!-- <view class="model_img">
					<text class="money">{{packObj.amount}}<text class="font">元</text></text>
					<text class="flabel">{{packObj.name}}</text>
				</view> -->
				<!-- <image class="model_button" @click="toList" src="../../static/coupons/model_button.png"></image> -->
			</view>
		</u-popup>
	</view>
</template>
...
<script>
import { communityBannerListApi } from '@/api/index.js';
...
export default {
	...
	methods: {
		...
		async getBannerList() {
			try {
				let res = await communityBannerListApi(this.reqParams);
				this.bannerList = res.data;
				this.isbanner = true;
				res.data.map(item => {
					console.log(item, 'item');
					this.bannerData.push(item.imgs);
				});
			} catch (e) {
		...
		swiperClick(e) {
			// 跳转
			jump(this.bannerList[e].linkUrl);
		},
		viewCoupon() {
			uni.navigateTo({
...
.content {
	...
	.followStyle {
		...
		background: linear-gradient(180deg, rgba(98, 98, 98, 1) 0%, rgba(153, 153, 153, 1) 100%);
		border-radius: 28rpx;
		z-index: 999;
		overflow: hidden;
		text-overflow: ellipsis;
...
.model {
	...
	&_img {
		width: 690rpx;
		// height: 552rpx;
		// background: url("../../static/coupons/model.png") no-repeat right top;
