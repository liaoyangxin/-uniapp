<template>
	<view>
		...
		<view class="footer flex-no-horizontal">
			...
			<text class="footer-agreement font-14">《用户使用协议》</text>
			<text class="footer-text font-14">、</text>
			<text class="footer-agreement font-14">《隐私政策》</text>
		</view>
	...
	methods: {
		...
		async nowLoginClick() {
			try {
				...
				this.btnText = '登录';
				setTimeout(() => {
					if(uni.getStorageSync('exchangeCode')){
						uni.navigateTo({
							url: '/pages/user/exchange/exchange?exchangeCode=' + uni.getStorageSync('exchangeCode')
						})
					}else{
						uni.switchTab({
		...
		async getCode() {
			try {
				if (this.$refs.uCode.canGetCode) {
					...
					if (this.phone === '' && this.phone.length !== 11 && reg.test(this.phone)) {
